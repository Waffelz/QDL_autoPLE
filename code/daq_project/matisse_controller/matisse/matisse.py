...
from matisse_controller.wavemaster import WaveMaster, WA1600, WS7


class Matisse:
    matisse_lock = threading.Lock()

    def __init__(self, wavemeter_type: str = 'WaveMaster', wavemeter_port=None):
        try:
            ...
            self._force_large_scan = False  # Vasilis edited this from True to False
            ...
            if wavemeter_type == 'WA1600':
                self._wavemeter = WA1600(wavemeter_port)
            elif wavemeter_type == 'WS7':
                self._wavemeter = WS7()
            else:
                if wavemeter_port is None:
                    wavemeter_port = cfg.get(cfg.WAVEMETER_PORT)
                self._wavemeter = WaveMaster(wavemeter_port)
    ...
    def query(self, command: str, numeric_result=False, raise_on_error=True):
        ...
        try:
            ...
            # return self.query(command, numeric_result, raise_on_error)
    ...
    def set_wavelength(self, wavelength: float):
        ...
        Vasilis edit: We do not want to do the first step anymore
        ...
        2. Set approx. wavelength using BiFi. This is supposed to be good to about +-1 nm but it's usually very far off.
           Vasilis edit: This is why we will be skipping this.
        ...
        while True:
            ...
            if diff > cfg.get(cfg.LARGE_WAVELENGTH_DRIFT) or self._force_large_scan:
                ...
                # Vasilis edit: We are at a close wavelength most of the time. This is redundant.
    ...
    def reset_motors(self):
        ...
        self.reset_bifi_motor()
        self.reset_thin_etalon_motor()

    def reset_bifi_motor(self):
        """Move the birefringent filter to its configured reset position through a maximum allowed step"""
        old_pos = int(self.query('MOTBI:POS?', numeric_result=True))
        target_pos = cfg.get(cfg.BIFI_RESET_POS)
        max_step = cfg.get(cfg.BIFI_RESET_POS_MAX_STEP)
        direction = np.sign(target_pos - old_pos)
        while old_pos != target_pos:
            if abs(old_pos-target_pos) > max_step:
                old_pos = old_pos + direction*max_step
                self.query(f"MOTBI:POS {old_pos}")
            else:
                old_pos = target_pos
                self.query(f"MOTBI:POS {target_pos}")
            while old_pos != int(self.query('MOTBI:POS?', numeric_result=True)):
                time.sleep(0.1)

    def reset_thin_etalon_motor(self):
        """Move the thin etalon to its configured reset position through a maximum allowed step"""
        old_pos = int(self.query('MOTTE:POS?', numeric_result=True))
        target_pos = cfg.get(cfg.THIN_ETA_RESET_POS)
        max_step = cfg.get(cfg.THIN_ETA_RESET_POS_MAX_STEP)
        direction = np.sign(target_pos - old_pos)
        while old_pos != target_pos:
            if abs(old_pos-target_pos) > max_step:
                old_pos = old_pos + direction*max_step
                self.query(f"MOTTE:POS {old_pos}")
            else:
                old_pos = target_pos
                self.query(f"MOTTE:POS {target_pos}")
            while old_pos != int(self.query('MOTTE:POS?', numeric_result=True)):
                time.sleep(0.1)
