#!/usr/bin/env python3
"""
WS7 smoke test (direct driver) selecting Air/Vacuum output via SetResultMode.

Run:
  python script/test_ws7.py --n 200 --dt 0.02 --medium air
  python script/test_ws7.py --n 200 --dt 0.02 --medium vac
"""
...
from functions26.instruments.ws7.wlmConst import cReturnWavelengthAir, cReturnWavelengthVac
...
def main():
    ...
    ap.add_argument("--medium", choices=["air", "vac"], default="air",
                    help="Ask WS7 to return wavelength in air (default) or vacuum.")
    args = ap.parse_args()

    ws7 = WS7()

    # Set the WS7 output mode once
    mode = cReturnWavelengthAir if args.medium == "air" else cReturnWavelengthVac
    if hasattr(ws7.lib, "SetResultMode"):
        ws7.lib.SetResultMode(mode)
    ...
    if hasattr(ws7.lib, "GetResultMode"):
        print("ResultMode:", ws7.lib.GetResultMode())
