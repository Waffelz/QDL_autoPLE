...
class StabilizationThread(threading.Thread):
    def __init__(self, matisse, messages: Queue, *args, **kwargs):
        ...
        **kwargs
        ...
        rising_speed = self._matisse.query(f"SCAN:RISINGSPEED?", True)
        falling_speed = self._matisse.query(f"SCAN:FALLINGSPEED?", True)
        self.original_stabilization_rising_speed = rising_speed  # cfg.get(cfg.STABILIZATION_RISING_SPEED)
        self.original_stabilization_falling_speed = falling_speed  # cfg.get(cfg.STABILIZATION_FALLING_SPEED)

        self.set_original_stabilization_speeds()

    def set_original_stabilization_speeds(self):
        self._matisse.query(f"SCAN:RISINGSPEED {self.original_stabilization_rising_speed}")
        self._matisse.query(f"SCAN:FALLINGSPEED {self.original_stabilization_falling_speed}")

    def set_modified_stabilization_speeds(self, modifier: float):
        if self.original_stabilization_rising_speed / modifier > 0.0001:
            self._matisse.query(f"SCAN:RISINGSPEED {self.original_stabilization_rising_speed / modifier}")
            self._matisse.query(f"SCAN:FALLINGSPEED {self.original_stabilization_rising_speed / modifier}")
        else:
            self.set_original_stabilization_speeds()
        ...
        while True:
            if self.messages.qsize() == 0:
                ...
                if abs(drift) > cfg.get(cfg.LARGE_WAVELENGTH_DRIFT):
                    ...
                    if abs(drift) / cfg.get(cfg.STABILIZATION_TOLERANCE) < 3:
                        self.set_modified_stabilization_speeds(5)
                    else:
                        self.set_original_stabilization_speeds()
                    if drift > 0:
                        # measured wavelength is too high
                        print(f"Wavelength too high, decreasing. Drift is {drift} nm. Refcell is at "
                              f"{self._matisse.query('SCAN:NOW?', numeric_result=True)}")
                        ...
                        print(f"Wavelength too low, increasing.  Drift is {drift} nm. Refcell is at "
                              f"{self._matisse.query('SCAN:NOW?', numeric_result=True)}")
                        if not self._matisse.is_any_limit_reached():
                            if cfg.get(cfg.REPORT_EVENTS):
                                log_event(EventType.WAVELENGTH_DRIFT, self._matisse, current_wavelength,
